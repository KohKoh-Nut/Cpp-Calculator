...
public:
	...
	static void storeExpFormBracOptr(int optr_bef, int optr_af);

	//method to store new expression after formatting consecutive brackets
	static void storeExpFormConsBrac(int optr_bef, int optr_af, int mode);

	//method to store new expression after formatting brackets with orders right before
	static void storeExpFormBracOrders(int optr_bef, int optr_af);
